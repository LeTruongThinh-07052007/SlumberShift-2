...
import { useDeleteRestoreTastMutation } from "../redux/slices/api/taskApiSlice";
...
const Trash = () => {
  ...
  const deleteAllClick = () => {
    setType("deleteAll");
    setMsg("Bạn chắc chắn muốn xoá các nhiệm vụ này vĩnh viễn chứ?");
    setOpenDialog(true);
  };

  const restoreAllClick = () => {
    setType("restoreAll");
    setMsg("Bạn có chắc chắn muốn khôi phục tất cả hay không?");
    setOpenDialog(true);
  };
  ...
  const restoreClick = (id) => {
    setSelected(id);
    setType("restore");
    setMsg("Bạn có muốn khôi phục nhiệm vụ này không?");
    setOpenDialog(true);
  };
  ...
  const TableHeader = () => (
    <thead className='border-b border-gray-300'>
      <tr className='text-black  text-left'>
        ...
        <th className='py-2 line-clamp-1'>Được tạo vào</th>
      </tr>
    </thead>
  );
